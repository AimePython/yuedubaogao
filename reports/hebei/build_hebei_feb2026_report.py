# -*- coding: utf-8 -*-
"""河北2月市场运营分析报告：从 河北/2月 CSV 聚合现货日度序列，与《2026年2月河北南网电力市场信息报告》披露对齐，写入 HTML。"""
...
DATA_DIR = ROOT / "河北" / "2月"
JSON_PATH = DIR / "_hb_feb2026_spot.json"
TEMPLATE = DIR / "河北2月市场运营分析报告.html"
OUT_DIR = DIR / "dataflow" / "output"
OUT_SKILL = OUT_DIR / "河北2月市场运营分析报告.html"
PROC_META = DIR / "dataflow" / "processed" / "2026-02-meta.json"
SKILL_HB_ROOT = Path(r"c:\Users\wenjiel\Desktop\本家\河北\市场运营分析报告")

# 《2026年2月河北南网电力市场信息报告》「现货交易情况概述」
DISCLOSURE_FEB2026 = {
    "monthDaVolYi": 2.18,
    "monthRtVolYi": 105.64,
    "monthDaPrice": 357.57,
...
def _pick_csv(pattern: str) -> Path:
    matches = sorted(DATA_DIR.glob(pattern), key=lambda p: p.name)
    if not matches:
        raise SystemExit(f"未找到 CSV：{DATA_DIR / pattern}")
    preferred = [m for m in matches if " (2)" not in m.name and "(2)" not in m.name]
...
def aggregate_spot_from_csv() -> dict:
    da_p = _pick_csv("*日前结算电价*.csv")
    da_v = _pick_csv("*日前结算电量*.csv")
    rt_p = _pick_csv("*实时市场电价*.csv")
    rt_v = _pick_csv("*实时*结算电量*.csv")
    ...
    sda = sum(out["daVol"])
    srt = sum(out["rtVol"])
    target_da = DISCLOSURE_FEB2026["monthDaVolYi"] * 1e5
    target_rt = DISCLOSURE_FEB2026["monthRtVolYi"] * 1e5
    ...
    out["monthDaVolYi"] = DISCLOSURE_FEB2026["monthDaVolYi"]
    out["monthRtVolYi"] = DISCLOSURE_FEB2026["monthRtVolYi"]
    out["monthDaPrice"] = DISCLOSURE_FEB2026["monthDaPrice"]
    out["monthRtPrice"] = DISCLOSURE_FEB2026["monthRtPrice"]
...
def patch_html(html: str, d: dict) -> str:
    ...
    spread_s = f"{abs(spread_m):.3f}"
    vd_s, vr_s = f"{vd:.3f}", f"{vr:.3f}"

    if spread_m >= 0:
        price_line = (
            rf'2月实时均价 <span class="highlight">{mr_s}元/MWh</span>，'
            rf'日前均价 <span class="highlight">{mp_s}元/MWh</span>，实时较日前平均升水 <span class="highlight">{spread_s}元/MWh</span>。'
        )
    else:
        price_line = (
            rf'2月实时均价 <span class="highlight">{mr_s}元/MWh</span>，'
            rf'日前均价 <span class="highlight">{mp_s}元/MWh</span>，实时较日前平均贴水 <span class="highlight">{spread_s}元/MWh</span>。'
        )
  ...
  const daVol = {json.dumps(d["daVol"])};"""
    ...
    html = re.sub(
        r"<p><strong>💰 价格表现：</strong>[\s\S]*?</p>",
        f'<p><strong>💰 价格表现：</strong>{price_line}</p>',
        html,
        count=1,
    )

    html = re.sub(
        r"(<p><strong>⚡ 电量表现：</strong>)2月实时总电量 [\d.]+ 亿kWh，日前总电量 [\d.]+ 亿kWh[^<]*",
        rf"\g<1>2月实时总电量 {vr_s} 亿kWh，日前总电量 {vd_s} 亿kWh（与《2026年2月河北南网电力市场信息报告》「现货交易情况概述」披露一致；"
...
def main():
    ...
    meta = {
        "province": "河北",
        "period": "2026-02",
        "output_html": "reports/hebei/dataflow/output/河北2月市场运营分析报告.html",
        "sources": [
            "河北/2月/2026年2月河北南网电力市场信息报告.pdf",
            "河北/2月/指标数据明细-河北-省内现货-日前结算电价（2026-02-01~2026-02-28）.csv",
            "河北/2月/指标数据明细-河北-省内现货-日前结算电量（2026-02-01~2026-02-28）.csv",
            "河北/2月/指标数据明细-实时市场电价（2026-02-01~2026-02-28）.csv",
            "河北/2月/指标数据明细-河北_实时实时结算电量（2026-02-01~2026-02-28）.csv",
            str(ROOT / "_hb_feb_pdf_extract.txt"),
        ],
        "spot_from_csv": {
        ...
        "notes": [
            "市场主体、中长期与现货概述摘自河北南网信息报告PDF（仓库内 _hb_feb_pdf_extract.txt 摘要）；核心速览现货电量与均价与报告「现货交易情况概述」一致。",
